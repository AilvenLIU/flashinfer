...
// Standalone routing -> exported Frost FC1/SwiGLU -> exported Frost FC2 -> finalize.
...
class FrostMoePlan final : public tvm::ffi::ModuleObj {
 public:
  FrostMoePlan(Function fc1, Function fc2, int64_t tokens, int64_t hidden, int64_t intermediate,
               int64_t experts, int64_t topk, int device, size_t scratch1, size_t scratch2,
               bool scale_first)
      : fc1_(std::move(fc1)),
        fc2_(std::move(fc2)),
        t_(tokens),
  ...
  const char* kind() const final { return "frost_bf16_moe_plan"; }
 ...
 private:
  void run(TensorView out, TensorView x, TensorView ids, TensorView scores, TensorView w1,
    ...
    fc1_(problem1_, TensorView(&first), TensorView(&desc), TensorView(&tx), TensorView(&gate),
         TensorView(&up), TensorView(scale_first_ ? &factor : &tm),
         TensorView(scale_first_ ? &tm : &factor), static_cast<void*>(stream));
    checked(cudaMemsetAsync(scratch, 0, scratch2_, stream));
    dshape[0] = scratch2_ / 8;
    fc2_(problem2_, TensorView(&first), TensorView(&desc), TensorView(&tm), TensorView(&down),
         TensorView(&ty), static_cast<void*>(stream));
...
Module make_plan(Function fc1, Function fc2, int64_t tokens, int64_t hidden, int64_t intermediate,
  ...
  TVM_FFI_ICHECK_EQ(hidden % 8, 0) << "Frost MoE routing uses 128-bit BF16 packs";
  TVM_FFI_ICHECK(scratch1 >= 0 && scratch1 % 128 == 0 && scratch2 >= 0 && scratch2 % 128 == 0);
  return Module(tvm::ffi::make_object<FrostMoePlan>(std::move(fc1), std::move(fc2), tokens, hidden,
                                                    intermediate, experts, topk, device, scratch1,
                                                    scratch2, scale_first));
}
}  // namespace
