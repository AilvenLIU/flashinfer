"""Independent BF16 MoE engine built from sealed Frost FC1 and FC2 objects.
    ...
    def __init__(
        ...
        for a, b in product(first, second):
            ...
            plan = module.make_plan(
                runtime._load_object(a.object_path, a.object_sha256, a.symbol),
                runtime._load_object(b.object_path, b.object_sha256, b.symbol),
                tokens,
                hidden,
                intermediate,
                experts,
...
class FrostBf16MoeRunner(MoERunner):
    ...
    def _check_support(self):
        ...
        if self.device.type != "cuda" or get_compute_capability(self.device) != (10, 0):
            raise NotImplementedError("Frost BF16 MoE objects require SM100a")
        if self.config.activation != SwiGLU():
            raise NotImplementedError("Frost BF16 MoE supports only default SwiGLU")
        if not self.config.finalize.do_finalize:
            raise NotImplementedError("Frost BF16 MoE requires finalized output")
        if (
            self.config.quant.per_token_scale
            or self.config.quant.swizzled_scale_factors
        ):
            raise NotImplementedError(
                "Frost BF16 MoE does not consume quantization scales"
            )
    ...
    def _validate_pack(self, act, weights):
        if act.routing_input_mode not in self.supported_routing_modes:
            raise NotImplementedError("Frost BF16 MoE requires precomputed routing")
        x = act.hidden_states_q
        if x.ndim != 2 or x.dtype != torch.bfloat16:
            raise ValueError("Frost BF16 MoE requires BF16 x[T,H]")
        if act.hidden_states_scale is not None or act.per_token_scale is not None:
            raise ValueError("Frost BF16 MoE does not consume activation scales")
        t, h = x.shape
        ...
        if not (0 < t <= min(1 << 20, self.config.execution.tune_max_num_tokens)):
            raise ValueError("Frost BF16 MoE token count exceeds supported bounds")
        if not (0 < k <= e and t * k < 2**31):
            raise ValueError("Frost BF16 MoE requires int32 expanded row indices")
        ...
        if set(view) != _WEIGHT_KEYS:
            raise ValueError("Frost BF16 MoE requires plain weights without overrides")
        w1, w2 = view["fc1_expert_weights"], view["fc2_expert_weights"]
        if tuple(w1.shape) != (e, 2 * i, h) or tuple(w2.shape) != (e, h, i):
            raise ValueError("Frost BF16 MoE weight geometry mismatch")
        if any(w.dtype != torch.bfloat16 for w in (w1, w2)):
            raise ValueError("Frost BF16 MoE weights must be BF16")
        if any(v.device != self.device or not v.is_contiguous() for v in (x, w1, w2)):
            raise ValueError(
                "Frost BF16 MoE needs contiguous tensors on the runner device"
            )
        if any(v.data_ptr() % 16 for v in (x, w1, w2)):
            raise ValueError("Frost BF16 MoE requires 16B-aligned data")
    ...
    def accepts(self, act, weights):
        ...
        if not large_bf16_moe(self.config, act, 100):
            return False
        try:
    ...
    def pack_inputs(self, act, weights):
        ...
        if not first or not second:
            raise ValueError("No matching offline Frost FC1/FC2 objects")
        key = (
            t,
            h,
            i,
        ...
        if key not in self._plans:
            with torch.cuda.device(self.device):
                if torch.cuda.is_current_stream_capturing():
                    raise RuntimeError("Prepare Frost MoE outside CUDA Graph capture")
    ...
    def forward(
        ...
        if state is None:
            raise ValueError("Preserve packed Frost inputs or pass their launch_state")
        if tactic == -1:
            tactic = next(iter(state.launches))
        if tactic not in state.launches:
            raise ValueError(f"Unknown or stale Frost MoE tactic: {tactic!r}")
...
def automatic_candidate(config, device):
    ...
    runner = FrostBf16MoeRunner(config, device)
    try:
